...
function add_chat_events() {
  add_events("[msg-topic]", "click", function () {
    let body_field_value = this.parentNode.getElementsByClassName("body")[0].value
    let msg_field_value = JSON.parse(this.parentNode.getElementsByClassName("msg")[0].value)
...
function add_chat_container_events() {
  add_events("[load-chat-msgs]", "click", function () {
    ...
    this.onclick = null;
    let msg_field_value = JSON.parse(document.getElementById(this.innerText).getElementsByClassName("msg")[0].value)
    if (channel_user.state == "joined") {
...
function add_chat_room_events() {
  add_events("[delete_chat_room]", "click", function () {
    let msg_field_value = JSON.parse(document.getElementById(this.getAttribute("delete_chat_room")).getElementsByClassName("msg")[0].value)
    if (channel_user.state == "joined") {
