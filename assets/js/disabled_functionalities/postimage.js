...
if (typeof postimage.ready === "undefined") {
  ...
  postimage.opt.skip =
    postimage.opt.skip ||
    "recaptcha|username_list|search|recipients|coppa|^signature$|^forum_desc$|^req_maintenance_message$|^req_reason$|form";
  ...
  postimage.ui = "";
  postimage.ui +=
    typeof screen.colorDepth != "undefined" ? screen.colorDepth : "?";
  ...
  postimage.ui +=
    typeof navigator.systemLanguage != "undefined"
      ...
      : "?";
  postimage.ui +=
    typeof navigator.userLanguage != "undefined" ? navigator.userLanguage : "?";
  postimage.ui +=
    typeof postimage.ts.toLocaleString == "function"
      ? postimage.ts.toLocaleString()
      : "?";
  postimage.ui +=
...
postimage.upload = function (areaid) {
  ...
  if (typeof postimage.windows[areaid] !== typeof undefined) {
    ...
    if (
      postimage.windows[areaid].window !== typeof undefined &&
      postimage.windows[areaid].window
    ) {
      postimage.windows[areaid].window.close();
    }
  }
  postimage.windows[areaid] = {};
  postimage.windows[areaid].window = window.open(
    "https://" + postimage.opt.host + "/upload?" + params,
    areaid,
  ...
  postimage.windows[areaid].timer = window.setInterval(function () {
    if (
      self.windows[areaid] === typeof undefined ||
      !self.windows[areaid].window ||
      self.windows[areaid].window.closed !== false
    ) {
    ...
    (function (i, dz) {
      dz.activate = function () {
        ...
        area.style["backgroundImage"] =
          "url('https://postimgs.org/img/logo.png')";
      ...
      dz.on("queuecomplete", function (e) {
        ...
        xhr.open(
          "GET",
          "https://" + postimage.opt.host + "/mod?dz=1&" + params
        );
        xhr.onload = function () {
          if (xhr.status === 200) {
...
postimage.init = function () {
  ...
  for (var i = 0; i < areas.length; i++) {
    var area = areas[i];
    if (
      area.getAttribute("data-postimg") !== null ||
      !area.name ||
      (area.name && postimage.skip.test(area.name)) ||
      (area.id && postimage.skip.test(area.id))
    ) {
      continue;
    }
    ...
    postimage.insert(
      area,
      postimage.render("'" + area.getAttribute("data-postimg") + "'")
    );
  }
...
if (
  opener &&
  !opener.closed &&
  postimage.params.hasOwnProperty("postimage_id") &&
  postimage.params.hasOwnProperty("postimage_text")
) {
  postimage.output(
    postimage.params["postimage_id"],
    postimage.params["postimage_text"],
    true
  );
} else {
  if (typeof window.addEventListener == "function") {
