defmodule StoreHallWeb.ItemChannelTest do
  ...
  setup do
    ...
    {:ok, _, socket} = subscribe_and_join(socket, ItemsChannel, "/items/#{item.id}")

    {:ok, guest_socket} = connect(UserSocket, %{"token" => "guest"})
    {:ok, _, guest_socket} = subscribe_and_join(guest_socket, ItemsChannel, "/items/#{item.id}")

    {:ok, socket: socket, guest_socket: guest_socket, item: item, user: user}
  end
